...
export default async function ProductDetailPage({
  ...
  return (
    <div className="mx-auto w-full max-w-4xl flex-1 px-4 py-10 sm:px-6">
      <Link
        href="/"
        className="mb-6 inline-flex items-center gap-1 text-sm font-medium text-indigo-600 hover:underline"
      >
        ← Volver al catálogo
      </Link>

      <div className="grid grid-cols-1 gap-8 rounded-2xl border border-black/10 bg-white p-6 sm:grid-cols-2 dark:border-white/10 dark:bg-zinc-900">
        {/* eslint-disable-next-line @next/next/no-img-element -- imagen externa de DummyJSON */}
        <img
          src={product.images?.[0] ?? product.thumbnail}
          alt={product.title}
          className="aspect-square w-full rounded-xl bg-zinc-100 object-cover dark:bg-zinc-800"
        />
